...
import ClientUpload from '../upload/clientUpload'
import ServerDeploy from '../upload/serverDeploy'
import ServerUpdade from '../upload/serverUpdate'
import ServerLog from '../upload/serverLog'
        ...
        <Row style={{ height: '100%' }}>
          <Col span={23} push={1} style={{ height: '100%' }}>
            <Layout style={{ height: '100%', width: '100%', position: 'relative' }} >
              <div style={{ height: '100%', width: '100%', zIndex: this.state.index[1], position: 'absolute' }}>
                <Sys />
              </div>
              <div style={{ height: '100%', width: '100%', zIndex: this.state.index[2], position: 'absolute' }}>
                <List />
              </div>
              <div style={{ height: '100%', width: '100%', zIndex: this.state.index[3], position: 'absolute' }}>
                <ClientUpload />
              </div>
              <div style={{ height: '100%', width: '100%', zIndex: this.state.index[4], position: 'absolute' }}>
                <ServerDeploy />
              </div>
              <div style={{ height: '100%', width: '100%', zIndex: this.state.index[5], position: 'absolute' }}>
                <ServerUpdade />
              </div>
              <div style={{ height: '100%', width: '100%', zIndex: this.state.index[6], position: 'absolute' }}>
                <ServerLog />
              </div>
          ...
          <Col span={1} pull={23} style={{ height: '100%', zIndex: 2 }}>
            ...
            >
              ...
              <Menu theme="dark" defaultSelectedKeys={['1']} mode="inline" onClick={this.handleClick}>
                ...
                >
                  <Menu.Item key="4">部署</Menu.Item>
                  <Menu.Item key="5">重启/停止</Menu.Item>
